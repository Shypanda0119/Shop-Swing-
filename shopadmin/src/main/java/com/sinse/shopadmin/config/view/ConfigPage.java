...
public class ConfigPage extends Page {
	...
	// 목록 가져오기
	public void size_getList() {
		List<Size> sizeList = sizeDAO.selectAll();
		
		//Collection framework에 최적화 된 개선된 for문 improved for java 5버전
		Vector vec = new Vector(sizeList); //ArrayList와 동일하지만, 쓰레드 동기화 지원..(내부적 연산이 들어가서 속도 느림)
