 ...
 * 이 테이블에 대한 CRUD(Insert, Update, Read=select, Delete)를 무조건 만든다..
 * 예를 들어, 협업관계에 있는 같은 팀 프로젝트 구성원인 A, B가 있다고 가정했을때,
 * Product 테이블에 대해서 A가 모델, DAO를 만들면 B는 그 객체를 가져가 사용하면 됨.. 중복 정의 x
 */
//이 코드는 오직!!! 데이터 베이스에 관련된 코드만 들어있어야 함.. 따라서 Swing제어 등의 디자인 코드가
//절대로 들어있으면 안됨!!! 왜? 객체는 MVC에서 영역 중, Model 영역의 즉 로직 객체 영역을 담당하므로
//View가 섞여있으면 안됨
public class TopCategoryDAO {
	// 아래의 코드의 동작 시점? dbManager 인스턴스 변수이므로, 누군가가 new TopCategory()를 호출할때 초기화하면서
	// DBManager이 초기화 된다.
	DBManager dbManager = DBManager.getInstance();

	public TopCategoryDAO() {
		// DBManager은 싱글턴 패턴으로 정의되어 있으므로, new 할 수 없다.
	...
	public ArrayList selectAll() {
		...
		ResultSet rs = null;
		ArrayList list = new ArrayList<>();//js배열과 아주 흡사함..
		...
		try {
			...
			//만일 배열로 rs의 데이터를 담게되면, 배열의 크기를 명시해야 하는 이유로, rs.last(), getRow(), rs.beforeFirst()
